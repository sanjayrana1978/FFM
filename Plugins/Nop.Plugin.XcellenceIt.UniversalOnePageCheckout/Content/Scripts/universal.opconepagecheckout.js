...
var PaymentMethod = {
    ...
    toggleUseRewardPoints: function (useRewardPointsInput) {
        if (useRewardPointsInput.checked) {
            ...
            $('#checkout-payment-info-load').hide();
        }
        else {
            $('#payment-method-block').show();
            $('#checkout-payment-info-load').show();
        }
        this.save();
    },
    ...
    nextStep: function (response) {
        ...
        if (response.goto_section == 'confirm_order') {
            //console.log('hi-1')
            window.CommonDefaults.paymentMethodRequired = false;
        } else {
            //console.log('hi-2')
            window.CommonDefaults.paymentMethodRequired = true;
        }
    }
};
...
var PaymentInfo = {
    ...
    save: function () {
        if (!window.CommonDefaults.paymentMethodRequired) {
            displayAjaxLoading(false);
            ConfirmOrder.save();
            return false;
        }
    ...
    resetLoadWaiting: function () {
        ...
        if (!window.CommonDefaults.paymentMethodRequired) {
            ConfirmOrder.save();
            //console.log(window.CommonDefaults.paymentMethodRequired);
        }
    },
...
var ConfirmOrder = {
    ...
    save: function () {
        if (Checkout.loadWaiting != false) return;
        if (window.CommonDefaults.paymentMethodRequired == false) {
            //terms of service
