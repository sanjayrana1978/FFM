...
{
    public partial class OPCShoppingCartController : BasePublicController
    {
        #region Fields
        ...
        private readonly UniversalOnePageCheckoutSettings _universalOnePageCheckoutSetting;
        ...
        public OPCShoppingCartController(CaptchaSettings captchaSettings,
            ...
            UniversalOnePageCheckoutSettings universalOnePageCheckoutSetting,
            ...
            IHtmlFormatter htmlFormatter)
        {
            ...
            this._universalOnePageCheckoutSetting = universalOnePageCheckoutSetting;
        ...
        protected virtual async Task<string> ParseProductAttributesAsync(Product product, IFormCollection form, List<string> errors)
        {
            //product attributes
            ...
            bool loadPicture, IFormCollection form)
        {
            ...
            if (await _permissionService.AuthorizeAsync(StandardPermissionProvider.DisplayPrices) && !product.CustomerEntersPrice)
            {
                ...
                basepricepangv = await _priceFormatter.FormatBasePriceAsync(product, finalPriceWithDiscountBase);
            }
        ...
        public virtual async Task<IActionResult> Cart()
        {
            ...
            {
                var setting = await _settingService.LoadSettingAsync<UniversalOnePageCheckoutSettings>((await _storeContext.GetCurrentStoreAsync()).Id);
                if (!setting.DisableShoppingCart)
                {
                    //if Cart Page is Enable
                    return View(model);
                }
                else
                {
        ...
        [HttpPost]
        public virtual async Task<IActionResult> OPCUpdateCart(IFormCollection form)
        {
            if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.EnableShoppingCart))
                return RedirectToRoute("HomePage");
            ...
            if (cart.Count == 0)
            {
                ...
                {
                    success = true,
                    returnUrl = Url.RouteUrl("ShoppingCart")
                });
            }
            ...
            {
                success = true,
                carthtml = await RenderPartialViewToStringAsync("OPCUpdateCart", model),
                updatetopcartsectionhtml,
                updateflyoutcartsectionhtml
            });
        }
        ...
        [HttpGet]
        public virtual async Task<IActionResult> GetFlayoutCartHtml()
        {
            var cart = await _shoppingCartService.GetShoppingCartAsync(await _workContext.GetCurrentCustomerAsync(), ShoppingCartType.ShoppingCart, (await _storeContext.GetCurrentStoreAsync()).Id);
            //display notification message and update appropriate blocks
            var updatetopcartsectionhtml = string.Format(await _localizationService.GetResourceAsync("ShoppingCart.HeaderQuantity"),
            ...
            var updateflyoutcartsectionhtml = _shoppingCartSettings.MiniShoppingCartEnabled
                ? await this.RenderViewComponentToStringAsync("FlyoutShoppingCart")
                : "";
